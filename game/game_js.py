...
class JSTools:
    ...
    """
    WASM compatible Javascript tools
    """
    ...
    async def getLoginUser(self):
        req = RequestHandler()
        self.print("Get Login User")
        url = "/getloginuser"
        return RequestHandler.get(req, url=url)
    
    async def postScore(self, data, newScore, previousHighScore):
        url = "/savescore"
        if data is None:
            data = {}
        
        responseData = req.post(req, url=url, data=data)
        
        if self.is_emscripten:
            parent = platform.window.parent
            scoreTable = parent.document.getElementById("user_scores")
            scoreTable.innerHTML = responseData
            scoreTable.style.display = "block"

            if self.debug:
                self.print("new score: " + str(newScore))
                self.print("Last high score: " + str(previousHighScore))

            if newScore > previousHighScore:
                highScoreDiv = parent.document.getElementById("user_high_score")
                highScoreDiv.innerHTML = "You have achieved a new high score: " + str(newScore)
            ...
            if self.debug:
                self.print(responseData)
            self.result = responseData
        return self.result
    
    async def clearHighScore(self):
        if self.is_emscripten:
            await asyncio.sleep(0)
            parent = platform.window.parent
            if self.debug:
                self.print("Clearing high score")
